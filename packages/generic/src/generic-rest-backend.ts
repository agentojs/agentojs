/**
 * GenericRESTBackend — adapter for arbitrary REST APIs (car dealers, wholesalers, etc.).
 *
 ...
 * GenericBackendNotImplementedError when the remote API does not support them.
 */
...
import type { GenericEndpointsMap, GenericRESTBackendConfig } from './types.js';
...
export class GenericBackendNotImplementedError extends Error {
  constructor(method: string) {
    super(
      `Generic REST backend does not support "${method}". ` +
        `Configure the remote API to provide this endpoint, or use a specialized backend (medusa, woocommerce).`,
    );
    this.name = 'GenericBackendNotImplementedError';
  }
}

// ─── Implementation ──────────────────────────────────────────────

export class GenericRESTBackend implements CommerceBackend {
  ...
  constructor(config: GenericRESTBackendConfig) {
  ...
  async createPaymentSessions(cartId: string): Promise<Cart> {
    throw new GenericBackendNotImplementedError(

...
import { WooCommerceProvider, WooCommerceApiError } from '../woocommerce-provider.js';
import type { WooCommerceProviderConfig, WcStoreProduct, WcCart, WcOrder } from '../woocommerce-provider.js';

const BASE_URL = 'https://shop.example.com';
const CONSUMER_KEY = 'ck_test_123';
const CONSUMER_SECRET = 'cs_test_456';

function makeConfig(overrides?: Partial<WooCommerceProviderConfig>): WooCommerceProviderConfig {
...
describe('WooCommerceProvider', () => {
  ...
  it('constructor creates instance', () => {
    const backend = new WooCommerceProvider(makeConfig());
    expect(backend).toBeInstanceOf(WooCommerceProvider);
  });
  ...
  it('strips trailing slash from baseUrl', () => {
    ...
    const backend = new WooCommerceProvider(makeConfig({ baseUrl: 'https://shop.example.com/' }));

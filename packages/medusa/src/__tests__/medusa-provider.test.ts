...
import { MedusaProvider, MedusaApiError } from '../medusa-provider.js';
import type { MedusaProviderConfig } from '../medusa-provider.js';

const BASE_URL = 'https://medusa.example.com';
const API_KEY = 'pk_test_123';

function makeConfig(overrides?: Partial<MedusaProviderConfig>): MedusaProviderConfig {
...
describe('MedusaProvider', () => {
  ...
  it('constructor creates instance', () => {
    const backend = new MedusaProvider(makeConfig());
    expect(backend).toBeInstanceOf(MedusaProvider);
  });
  ...
  describe('searchProducts', () => {
    it('calls /store/products with search query and region', async () => {
      ...
      const backend = new MedusaProvider(makeConfig({ regionId: 'reg_eu' }));
    ...
    it('auto-resolves region when not provided', async () => {
      ...
      const backend = new MedusaProvider(makeConfig({ regionId: undefined }));
    ...
    it('passes category_id[] and collection_id[] params', async () => {
      ...
      const backend = new MedusaProvider(makeConfig({ regionId: 'reg_1' }));
  ...
  describe('headers', () => {
    ...
    it('omits api key header when empty', async () => {
      ...
      const backend = new MedusaProvider(makeConfig({ apiKey: '' }));
      await backend.getCollections();
